...
def _undo_move(target: str) -> None:
    if _NUL not in target:
        raise ValueError(
            f"batch_undo_move: malformed target (no NUL separator): {target!r}"
        )
...
def _undo_move_overwrite(target: str) -> None:
    ...
    if len(parts) != 3:
        raise ValueError(
            f"batch_undo_move_overwrite: malformed target (expected 3 NUL-separated "
            f"parts): {target!r}"
        )
    dest, src, snap = parts
...
def _undo_snapshot_restore(target: str) -> None:
    if _NUL not in target:
        raise ValueError(
            f"batch_undo_snapshot_restore: malformed target (no NUL separator): "
            f"{target!r}"
        )
...
def _undo_log_append(target: str) -> None:
    ...
    if len(parts) != 3:
        raise ValueError(
            f"batch_undo_log_append: malformed target (expected 3 NUL-separated "
            f"parts): {target!r}"
        )
    live, snap, existed = parts
    if existed not in ("0", "1"):
        raise ValueError(
            f"batch_undo_log_append: malformed 'existed' token "
            f"(expected '0' or '1', got {existed!r}): {target!r}"
        )
    live_p = Path(live)
    snap_p = Path(snap)
...
def commit(
    *,
    ...
    # Case-insensitive dedupe so APFS / NTFS don't let "X.md" and "x.md" both
    # land at the same on-disk inode and silently clobber each other.
    seen: set[str] = set()
    for m in plan.moves:
        key = m.to.lower()
        if key in seen:
    ...
    for cw in plan.concept_writes:
        key = cw.path.lower()
        if key in seen:
            raise ValueError(f"duplicate destination in plan: {cw.path!r}")
        seen.add(key)
    for aw in plan.author_writes:
        key = aw.path.lower()
        if key in seen:
            raise ValueError(f"duplicate destination in plan: {aw.path!r}")
        seen.add(key)

    try:
        for m in plan.moves:
            src = _safe_in_run_dir(run_dir, m.from_)
        ...
        try:
            ...
            log.error(
                "rollback FAILED while handling commit error %r — vault may "
                "be in inconsistent state",
                original, exc_info=rollback_err,
            )
